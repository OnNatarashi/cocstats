...
use BackendBundle\Entity\Batiment;
...
class BatimentController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $batiments = $em->getRepository('BackendBundle:Batiment')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('BackendBundle\Form\batimentType', $batiment);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $file = $batiment->getArchitecture();
            $fileName = md5(uniqid()).'.'.$file->guessExtension();
            $file->move(
                "uploads",
                $fileName
            );
            $batiment->setArchitecture($fileName);
            $em = $this->getDoctrine()->getManager();
            $em->persist($batiment);
            $em->flush();
    ...
    {
        $deleteForm = $this->createDeleteForm($batiment);
        $editForm = $this->createForm('BackendBundle\Form\batimentType', $batiment);
